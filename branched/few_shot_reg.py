...
def get_few_shot_datasets(args):
    ...
    trainval_loader = DataLoader(trainval_dataset, batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers)
...
def main(args):
    ...
    if args.baseline:
        results_file = open(os.path.join("results", "few_shot", args.test_dataset + "_" + str(args.shot_size) + "_baseline.txt"), 'w')
    else:
        results_file = open(os.path.join("results", "few_shot", args.test_dataset + "_" + str(args.shot_size) + ".txt"), 'w')
    ...
    for t in tqdm(range(args.num_tasks)):
        ...
        if args.baseline:
            ...
            clf, C = set_params(clf, torch.tensor(best_params["C"]), dataset_info[args.test_dataset]['mode'])
